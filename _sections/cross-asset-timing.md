...
shifted by each asset's own historical lag-vs-BTC-bottom (SOL's *price* shape borrows from ETH — see footnote 4). Macro assets are cycle-tied since
...
| 4 | **SOL** | 2026-09-30 → 2027-01-28 | $77.80 – $85.96 | 2029-03-07 → 2029-07-21 | $213.96 – $1,585.10 | Lags BTC by ~3 months — only crypto asset with a later B4 than BTC. <sup>4</sup> |
...
<ol class="table-dense-footnotes">
  ...
  <li><sup>4</sup> **SOL.** Cross-check: BTC-borrowed timing (B4 window = BTC B4 + SOL lag). Price shape: <code>borrowed_2_stage_from_ETH</code> — SOL's own dd/mult series are dominated by its C3 first-cycle monster (mult=502x), so ETH's per-cycle ratios are borrowed aligned by asset-cycle ordinal (SOL C3~ETH C2, SOL C4~ETH C3, SOL C5~ETH C4); ETH fit evaluated at ordinal 3 (dd=0.687, mult=7.11).</li>
